...
public final class PluginPrefs {
    ...
    public static void setClosedHistoryLimit(int limit) {
        ...
        if (current.size() > limit) saveClosedHistory(current.subList(0, limit));
    }
    ...
    public static java.util.List<ClosedEntry> getClosedHistory() {
        ...
        try {
            ...
            for (String k : rawKeys) {
                try { indices.add(Integer.parseInt(k)); }
    ...
    public static void pushClosedEntry(ClosedEntry entry) {
        ...
        if (entry.hasPath()) list.removeIf(e -> e.path.equals(entry.path));
        list.add(0, entry);
    ...
    public static void removeClosedEntry(String path) {
        java.util.List<ClosedEntry> list = getClosedHistory();
        if (list.removeIf(e -> e.path.equals(path))) saveClosedHistory(list);
    }
    ...
    public enum TagColor {
        ...
        TagColor(java.awt.Color c) { this.color = c; }
    }
    ...
    public static TagColor getTag(String tcKey) {
        ...
        if (v == null) return TagColor.NONE;
        try { return TagColor.valueOf(v); }
        catch (IllegalArgumentException ignored) { return TagColor.NONE; }
    }
    ...
    public static void setTag(String tcKey, TagColor tag) {
        ...
        if (tag == null || tag == TagColor.NONE) node.remove(tcKey);
        else node.put(tcKey, tag.name());
        flush(node);
    }
    ...
    public static String keyFor(org.openide.windows.TopComponent tc) {
        ...
        if (tip != null && !tip.trim().isEmpty()) {
            ...
            if (!tip.isEmpty()) return truncate(tip);
        }
        String name = OpenFilesTopComponent.resolveDisplayNameStatic(tc);
        if (name != null && !name.isEmpty()) return truncate(name);
    ...
    public static void setPins(java.util.List<String> ordered) {
        try {
            Preferences node = Preferences.userRoot().node(NODE_PINS);
            node.clear();
            for (int i = 0; i < ordered.size(); i++) node.put(String.valueOf(i), ordered.get(i));
            flush(node);
        } catch (BackingStoreException ignored) {}
    }
    ...
    public static Map<String, java.util.List<String>> getGroups() {
        ...
        try {
            ...
            for (String k : gNode.keys()) {
                if (!result.containsKey(k)) result.put(k, splitKeys(gNode.get(k, "")));
            }
        } catch (BackingStoreException ignored) {}
        return result;
    }
    ...
    public static void renameGroup(String oldName, String newName) {
        ...
        Map<String, java.util.List<String>> groups = getGroups();
        if (!groups.containsKey(oldName) || groups.containsKey(newName)) return;
        ...
        if (wasCollapsed) setGroupCollapsed(newName, true);
    }
    ...
    public static String groupOf(String tcKey) {
        for (Map.Entry<String, java.util.List<String>> e : getGroups().entrySet()) {
            if (e.getValue().contains(tcKey)) return e.getKey();
        }
        return null;
    }
    ...
    public static Set<String> allGroupedKeys() {
        Set<String> result = new LinkedHashSet<>();
        for (java.util.List<String> keys : getGroups().values()) result.addAll(keys);
        return result;
    }
    ...
    public static void setGroupCollapsed(String sectionName, boolean collapsed) {
        ...
        if (collapsed) node.putBoolean(k, true);
        else           node.remove(k);
        flush(node);
    }

    public static void setAllGroupsCollapsed(boolean collapsed) {
        for (String name : getGroupOrder()) setGroupCollapsed(name, collapsed);
    ...
    private static void flush(Preferences node) {
        try { node.flush(); } catch (BackingStoreException ignored) {}
    }
}