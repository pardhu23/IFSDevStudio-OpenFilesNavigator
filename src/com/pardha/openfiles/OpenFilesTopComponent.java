...
public final class OpenFilesTopComponent extends TopComponent {
        ...
        if (TopComponent.Registry.PROP_OPENED.equals(prop)) {
            ...
            for (TopComponent tc : allEditors) {
                if (!nowOpen.contains(tc)) {
                    ...
                    if ("IFS Start Page".equals(name)) continue;
        ...
        fileTree.addMouseListener(new MouseAdapter() {
            @Override public void mousePressed(MouseEvent e) {
                ...
                if (node.getUserObject() instanceof String) {
                    ...
                    if (sectionName.startsWith("⊞ ")) sectionName = sectionName.substring(2);
    ...
    private void reopenClosedEntry(PluginPrefs.ClosedEntry entry) {
        ...
        try {
            ...
            if (!file.exists()) { showReopenError(entry.name); return; }
    ...
    private void autoExpandClosedStrip() {
        if (!closedStripBody.isVisible()) toggleClosedStrip();
    ...
    private void openSettingsDialog() {
        ...
        extScroll.setMaximumSize(new Dimension(Integer.MAX_VALUE, 72));
        JLabel extHint = new JLabel("Comma-separated, e.g.  entity,projection,plsql,java");
        ...
        noteBg.add(tooltipRb); noteBg.add(inlineRb); noteBg.add(subtitleRb);
        ...
        content.add(tooltipRb); content.add(inlineRb); content.add(subtitleRb);
        content.add(Box.createVerticalStrut(12));

        addSectionLabel(content, "Generated (build) files in tree view:");
        ButtonGroup buildBg = new ButtonGroup();
        JRadioButton buildInlineRb  = new JRadioButton("Show inline in their folder (greyed/italic)");
        JRadioButton buildGroupedRb = new JRadioButton("Group under a \u201CGenerated\u201D folder at the bottom");
        ...
        closedLimitRow.add(new JLabel(" files  (1\u2013" + PluginPrefs.CLOSED_LIMIT_MAX + ")"));
    ...
    private void addFileActionsToMenu(JPopupMenu menu,
        ...
        if (inPinnedSection) {
            if (single) {
                ...
                menu.add(menuItem("Unpin", e -> { PluginPrefs.removePin(tcKey); refreshList(); }));
            } else {
        ...
        if (single && !PluginPrefs.isBuildFile(targets.get(0))) {
            ...
            if (hasNote) {
                menu.add(menuItem("Clear note", e -> {
                    PluginPrefs.removeNote(tcKey); refreshList();
                }));
            }
        }
    ...
    private boolean isCoreFile(TopComponent tc) {
        String tip = tc.getToolTipText();
        if (tip == null || tip.trim().isEmpty()) return false;
    ...
    private boolean isEditorTC(TopComponent tc) {
        ...
        if ("IFS Start Page".equals(displayName)) return false;
    ...
    private FileObject remapToModelFile(FileObject fo) {
        ...
        String[] targetExts;
        switch (ext) {
            case "plsql": case "views": case "storage":
                targetExts = new String[]{"entity", "utility"}; break;
            case "plsvc":
                targetExts = new String[]{"projection", "fragment"}; break;
            default: return fo;
        }
                ...
                try {
                    ...
                    while (cls != null) {
                        try {
                            ...
                            break;
                        } catch (NoSuchMethodException nsme) { cls = cls.getSuperclass(); }
                    }
                    if (m == null) {
    ...
    private java.io.File readCoreFilesRootForProject(java.io.File projectDir) {
        ...
        for (java.io.File propsFile : candidates) {
            ...
            if (!f.isAbsolute()) f = new java.io.File(projectDir, val.trim());
    ...
    private void runActionOnTargets(String category, String actionId,
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            for (TopComponent tc : targets) {
                if (tc.isOpened()) lookups.add(tc.getLookup());
            }
    ...
    public void componentOpened() {
        ...
        // FIX: seedInitialCoreRoots() calls OpenProjects.getOpenProjects() which
        // can block for several seconds on startup. The method name says "OffEdt"
        // but was being invoked directly on EDT here. Push to a background thread
        // so componentOpened() returns immediately and doesn't freeze the IDE.
        new Thread(() -> {
            QuickFileSearchDialog.seedInitialCoreRoots();
            // ensureIndexed() also starts a background thread internally, but it
            // reads cachedCoreRoots which we just populated, so the handoff must
            // be back on EDT to avoid a data race on the static Set.
            SwingUtilities.invokeLater(QuickFileSearchDialog::ensureIndexed);
    ...
    private static class OpenTreeCellRenderer extends DefaultTreeCellRenderer {
        ...
        public Component getTreeCellRendererComponent(
            ...
            if (userObj instanceof TopComponent) {
                ...
                if (!sel && isBuild) setForeground(OpenFilesCellRenderer.BUILD_FILE_COLOR);
        ...
        if (stashes.isEmpty()) {
            ...
            for (PluginPrefs.StashEntry stash : stashes) {
                ...
                sub.add(menuItem("Delete stash", e -> {
                    ...
                    if (res == JOptionPane.YES_OPTION) PluginPrefs.deleteStash(stash.name);
                }));
                menu.add(sub);
            }
        }
    ...
    private void stashTargets(java.util.List<TopComponent> targets) {
        ...
        if (items.size() > shown) sb.append("\u2026 and ").append(items.size() - shown).append(" more<br>");
    ...
    private void restoreStash(PluginPrefs.StashEntry stash) {
        ...
        for (PluginPrefs.StashItem item : stash.items) {
            if (!item.hasPath()) { skipped++; continue; }
            try {
                java.io.File file = FileUtil.normalizeFile(
    ...
    private void peekStash(PluginPrefs.StashEntry stash) {
        ...
        btnRow.add(closeBtn); btnRow.add(restoreBtn);
        restoreBtn.addActionListener(e -> { dlg.dispose(); restoreStash(stash); });
