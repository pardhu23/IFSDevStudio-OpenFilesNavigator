 ...
 *
 * <h3>EDT safety fixes</h3>
 * <ul>
 * <li>{@code seedInitialCoreRoots()} was calling {@code OpenProjects.getOpenProjects()}
 * on EDT inside {@code componentOpened()}. Fixed: dispatched to a background thread
 * before handing back to EDT.</li>
 * <li>{@code ensureIndexed()} was similarly routed through EDT. Now always background-first.</li>
 * <li>{@code windowListener} PROP_OPENED handler: {@code refreshList()} and
 * {@code rebuildClosedStrip()} are already deferred via {@code invokeLater} — kept as-is
 * but guarded against being called while a refresh is already pending.</li>
 * </ul>
 */
public final class OpenFilesTopComponent extends TopComponent {
   ...
   private void scheduleDelayedRefresh() {
      ...
      pendingRefresh = RP.post(
              () -> SwingUtilities.invokeLater(this::refreshList), 1500);
   }
         ...
         public void mousePressed(MouseEvent e) {
            ...
            if (SwingUtilities.isLeftMouseButton(e)) {
               if (item instanceof OpenFilesCellRenderer.GroupHeader) {
                  ...
                  final String secName = ((OpenFilesCellRenderer.GroupHeader) item).name;
            ...
            if (e.getKeyCode() == KeyEvent.VK_ENTER
               ...
               if (idx >= 0 && listModel.getElementAt(idx) instanceof OpenFilesCellRenderer.GroupHeader) {
                  final String secName
                          = ((OpenFilesCellRenderer.GroupHeader) listModel.getElementAt(idx)).name;
                  SwingUtilities.invokeLater(() -> toggleCollapse(secName));
               } else {
                  activateFromList();
               }
   ...
   private void rebuildClosedStrip() {
      ...
      for (PluginPrefs.ClosedEntry entry : history) {
         closedStripBody.add(buildClosedEntryRow(entry));
      }
   ...
   private void openSettingsDialog() {
      ...
      JTextArea extField = new JTextArea(PluginPrefs.getIndexExtensions(), 2, 0);
      extField.setLineWrap(true);
      extField.setWrapStyleWord(false);
      extField.setFont(new JTextField().getFont());
      ...
      extScroll.setMaximumSize(new Dimension(Integer.MAX_VALUE, 52));
      JLabel extHint = hint("Comma-separated, e.g.  entity,projection,plsql,java");
      content.add(extScroll);
      content.add(Box.createVerticalStrut(2));
      content.add(extHint);
      content.add(Box.createVerticalStrut(10));

      // ── 2-column grid: Note display | Generated files ─────────────────────
      String noteDisplay = PluginPrefs.getNoteDisplay();
      JRadioButton tooltipRb = new JRadioButton("Tooltip only");
      JRadioButton inlineRb = new JRadioButton("Inline indicator (\u270E suffix)");
      JRadioButton subtitleRb = new JRadioButton("Subtitle (below filename)");
      ...
      JRadioButton buildInlineRb = new JRadioButton("Inline (greyed/italic)");
      JRadioButton buildGroupedRb = new JRadioButton("Group under \u201CGenerated\u201D");
      ...
      JRadioButton treeFolderRb = new JRadioButton("Folder path");
      JRadioButton treeComponentRb = new JRadioButton("IFS component");
      ...
      JRadioButton sortAlphaRb = new JRadioButton("Alphabetical");
      JRadioButton sortOpenOrderRb = new JRadioButton("Open order");
      ...
      // Grid panel: 2 cols, each col is a titled sub-panel
      ...
      // Left: Note display | Tree group by
      addSubLabel(leftGrid, "Note display:");
      leftGrid.add(tooltipRb);
      leftGrid.add(inlineRb);
      leftGrid.add(subtitleRb);
      leftGrid.add(Box.createVerticalStrut(6));
      addSubLabel(leftGrid, "Tree view \u2014 group by:");
      leftGrid.add(treeFolderRb);
      leftGrid.add(treeComponentRb);

      // Right: Generated files | List sort
      addSubLabel(rightGrid, "Generated files (tree view):");
      rightGrid.add(buildInlineRb);
      rightGrid.add(buildGroupedRb);
      rightGrid.add(Box.createVerticalStrut(6));
      addSubLabel(rightGrid, "List view \u2014 sort by:");
      rightGrid.add(sortAlphaRb);
      rightGrid.add(sortOpenOrderRb);
      ...
      // ── Deploy order ──────────────────────────────────────────────────────
      addSectionLabel(content, "Generate & Deploy \u2014 extension order:");
      JTextArea deployOrderField = new JTextArea(PluginPrefs.getDeployOrder(), 2, 0);
      ...
      deployOrderScroll.setMaximumSize(new Dimension(Integer.MAX_VALUE, 52));
      JLabel deployHint = hint("Comma-separated. Deploys left-to-right; unlisted go last (A\u2013Z).");
      ...
      inlineRow1.setAlignmentX(Component.LEFT_ALIGNMENT);
      SpinnerNumberModel spinModel = new SpinnerNumberModel(
              PluginPrefs.getClosedHistoryLimit(), 1, PluginPrefs.CLOSED_LIMIT_MAX, 1);
      JSpinner limitSpinner = new JSpinner(spinModel);
      limitSpinner.setPreferredSize(new Dimension(52, limitSpinner.getPreferredSize().height));
      JLabel closedLabel = new JLabel("Recently closed: keep last ");
      closedLabel.setFont(closedLabel.getFont().deriveFont(Font.BOLD));
      inlineRow1.add(closedLabel);
      inlineRow1.add(limitSpinner);
      inlineRow1.add(new JLabel("  files  (1\u2013" + PluginPrefs.CLOSED_LIMIT_MAX + ")"));
      content.add(inlineRow1);
      content.add(Box.createVerticalStrut(6));

      JCheckBox autoScanCb = new JCheckBox(
              "File indexing: auto-scan and index files on IDE startup");
      autoScanCb.setSelected(PluginPrefs.isAutoScanEnabled());
      autoScanCb.setAlignmentX(Component.LEFT_ALIGNMENT);
      autoScanCb.setFont(autoScanCb.getFont().deriveFont(Font.BOLD));
      JLabel autoScanHint = hint("When off, indexing starts only when you open Quick File Search (Alt+P)");
      content.add(autoScanCb);
      content.add(Box.createVerticalStrut(2));
      content.add(autoScanHint);
      content.add(Box.createVerticalStrut(10));
      ...
      gScroll.setPreferredSize(new Dimension(260, 80));
      ...
      renGBtn.addActionListener(ev -> {
         ...
         if (newName != null && !newName.trim().isEmpty() && !newName.trim().equals(sel)) {
   ...
   private static void addSubLabel(JPanel panel, String text) {
      JLabel lbl = new JLabel(text);
   ...
   private void showGroupHeaderContextMenu(String groupName, int x, int y) {
      ...
      if (!isPseudo) {
         ...
         renameItem.addActionListener(e -> {
            ...
            if (newName != null && !newName.trim().isEmpty() && !newName.trim().equals(groupName)) {
      ...
      menu.add(menuItem(collapsed ? "Expand" : "Collapse", e -> toggleCollapse(groupName)));
   ...
   private void addFileActionsToMenu(JPopupMenu menu,
      ...
      if (single && !PluginPrefs.isBuildFile(targets.get(0))) {
         ...
         menu.add(menuItem(hasNote ? "Edit note\u2026" : "Add note\u2026", e -> {
            ...
            p.add(new JLabel("Note for " + resolveDisplayName(tc) + ":"), BorderLayout.NORTH);
            p.add(sp, BorderLayout.CENTER);
            int res = JOptionPane.showConfirmDialog(this, p,
                    "File Note", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
...

      {
         String currentTagName = single
                 ? PluginPrefs.getTag(PluginPrefs.keyFor(targets.get(0))).name() : null;
         ...
         for (PluginPrefs.TagColor tag : PluginPrefs.TagColor.values()) {
            ...
            tagItem.addActionListener(e -> {
               targets.forEach(tc -> PluginPrefs.setTag(PluginPrefs.keyFor(tc), chosen));
               refreshList();
            });
            tagMenu.add(tagItem);
         }
      ...
      if (!eligibleGroups.isEmpty()) {
         JMenu addToGroupMenu = new JMenu(single ? "Add to group" : "Add all to group");
         for (String gName : eligibleGroups) {
            addToGroupMenu.add(menuItem(gName, e -> {
               ...
               targets.forEach(tc -> PluginPrefs.addToGroup(gName, PluginPrefs.keyFor(tc)));
               refreshList();
            }));
         }
      ...
      if (inGroupSection != null) {
         if (single) {
            menu.add(menuItem("Remove from group \u201C" + inGroupSection + "\u201D", e -> {
               PluginPrefs.removeFromGroup(inGroupSection, PluginPrefs.keyFor(targets.get(0)));
               refreshList();
            }));
         } else {
            menu.add(menuItem("Remove all from group \u201C" + inGroupSection + "\u201D", e -> {
               targets.forEach(tc -> PluginPrefs.removeFromGroup(inGroupSection, PluginPrefs.keyFor(tc)));
               refreshList();
            }));
         }
      }
   ...
   private JMenuItem buildExecutePlsqlMenuItem(TopComponent tc) {
      try {
         ...
         tc.requestActive();
         java.lang.reflect.Method presenter = action.getClass().getMethod("getPopupPresenter");
   ...
   private JMenuItem buildExecutePlsqlMenuItemMulti(java.util.List<TopComponent> targets) {
      ...
      if (nativePresenter instanceof JMenu) {
         ...
         for (int i = 0; i < nativeMenu.getItemCount(); i++) {
            ...
            JMenuItem copy = new JMenuItem(src.getText());
            copy.addActionListener(e -> executeSequentially(targets));
            ourMenu.add(copy);
         }
         return ourMenu;
      }
      JMenuItem item = new JMenuItem("Execute PL/SQL Command (" + targets.size() + " files)");
   ...
   private void executeSequentially(java.util.List<TopComponent> targets) {
      ...
      PluginPrefs.sortByDeployOrder(ordered, this::resolveExtension, this::resolveDisplayName);
      RP.post(() -> {
   ...
   private void switchView(ViewMode mode) {
      viewMode = mode;
      PluginPrefs.setViewMode(mode == ViewMode.TREE ? PluginPrefs.VIEW_TREE : PluginPrefs.VIEW_LIST);
      CardLayout cl = (CardLayout) centerPanel.getLayout();
   ...
   private void registerListeners() {
      if (!listenersRegistered) {
         ...
         org.netbeans.api.project.ui.OpenProjects.getDefault()
                 .addPropertyChangeListener(projectListener);
   ...
   private void unregisterListeners() {
      if (listenersRegistered) {
         ...
         org.netbeans.api.project.ui.OpenProjects.getDefault()
                 .removePropertyChangeListener(projectListener);
   ...
   private void applyFilterToTree(java.util.List<TopComponent> filtered) {
      ...
      if (!pinnedList.isEmpty()) {
         DefaultMutableTreeNode pNode = new DefaultMutableTreeNode(PluginPrefs.SECTION_PINNED);
      ...
      for (Map.Entry<String, java.util.List<String>> entry : groupDef.entrySet()) {
         ...
         DefaultMutableTreeNode gNode = new DefaultMutableTreeNode("\u229E " + entry.getKey());
      ...
      if (buildGrouped) {
         ...
         if (!buildFiles.isEmpty()) {
            DefaultMutableTreeNode genNode = new DefaultMutableTreeNode(PluginPrefs.SECTION_GENERATED);
      ...
      for (int ci = 0; ci < root.getChildCount(); ci++) {
         DefaultMutableTreeNode child = (DefaultMutableTreeNode) root.getChildAt(ci);
   ...
   private void runActionOnEDT(String category, String actionId,
      ...
      javax.swing.Timer t = new javax.swing.Timer(300, e -> {
         ...
         if (action != null) {
            action.actionPerformed(new ActionEvent(tc, ActionEvent.ACTION_PERFORMED, ""));
         }
      });
      t.setRepeats(false);
      t.start();
      ...
      try {
         ...
         javax.swing.Timer t = new javax.swing.Timer(300, e -> {
            try {
               ...
               while (cls != null) {
                  try {
                     m = cls.getDeclaredMethod("performAction", org.openide.nodes.Node[].class);
                     break;
               ...
               if (m == null) {
                  System.err.println("[OpenFilesNavigator] CustomizeThis: performAction not found");
                  return;
               }
   ...
   private void runActionOnTargets(String category, String actionId,
      ...
      if (targets.size() == 1) {
         ...
         SwingUtilities.invokeLater(() -> {
            ...
            Action action = Actions.forID(category, actionId);
            if (action != null) {
               action.actionPerformed(new ActionEvent(single, ActionEvent.ACTION_PERFORMED, ""));
            } else {
               System.err.println("[OpenFilesNavigator] Action not found: " + category + "/" + actionId);
            }
         });
         return;
      }
            ...
            for (int i = 0; i < ordered.size(); i++) {
               ...
               SwingUtilities.invokeLater(() -> {
                  ...
                  Action ctx = (action instanceof ContextAwareAction)
                          ? ((ContextAwareAction) action).createContextAwareInstance(tc.getLookup())
                          : action;
                  ctx.actionPerformed(new ActionEvent(tc, ActionEvent.ACTION_PERFORMED, ""));
               });
               if (i < ordered.size() - 1) {
                  try {
                     Thread.sleep(800);
      ...
      SwingUtilities.invokeLater(() -> {
         ...
         action.actionPerformed(new ActionEvent(merged, ActionEvent.ACTION_PERFORMED, ""));
      });
   }
   ...
   public void componentOpened() {
      ...
      if (PluginPrefs.isAutoScanEnabled()) {
         new Thread(() -> {
            QuickFileSearchDialog.seedInitialCoreRoots();
            SwingUtilities.invokeLater(() -> {
               QuickFileSearchDialog.indexingEverStarted.set(true);
               QuickFileSearchDialog.scheduleCacheBuildsPublic(null);
            });
...
// =========================================================================
   private void showStashPopup(JButton anchor) {
      ...
      if (stashes.isEmpty()) {
         ...
         for (PluginPrefs.StashEntry stash : stashes) {
            JMenu sub = new JMenu("\uD83D\uDCCB " + stash.name + "  (" + stash.size() + ")");
   ...
   private void stashTargets(java.util.List<TopComponent> targets) {
      ...
      int confirm = JOptionPane.showConfirmDialog(this, sb.toString(),
              "Confirm Stash", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
   ...
   private void restoreStash(PluginPrefs.StashEntry stash) {
      ...
      for (PluginPrefs.StashItem item : stash.items) {
         ...
         try {
            java.io.File file = FileUtil.normalizeFile(
                    new java.io.File(item.absolutePath.replace('/', java.io.File.separatorChar)));
            if (!file.exists()) {
               skipped++;
               continue;
            }
   ...
   private void peekStash(PluginPrefs.StashEntry stash) {
      ...
      for (PluginPrefs.StashItem item : stash.items) {
         model.addElement(item.displayName + (item.hasPath() ? "" : "  \u26A0"));
      }
