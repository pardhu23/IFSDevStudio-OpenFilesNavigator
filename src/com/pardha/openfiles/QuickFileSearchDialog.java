 ...
 *
 * <p>
 * Press Ctrl+P → floating dialog appears → type filename (fuzzy matched) →
 * Enter or click to open. Escape dismisses.
 *
 * <p>
 * File cache: built once in a background thread when the dialog is first
...
public final class QuickFileSearchDialog extends JDialog {
   ...
   // ── Cache ──────────────────────────────────────────────────────────────
   // Core files (D:\IFS Workspace\...\checkout) are huge and change rarely.
   // They are cached STATICALLY — survive dialog close/reopen for the whole
   // IDE session. Only invalidated manually or if the core root path changes.
   private static List<FileEntry> coreCache = null;
   private static String coreCacheRoot = null;   // path the cache was built from
   private static volatile boolean coreCacheBuilding = false;

   // Cust layer files change frequently (developer edits).
   // Cached per-instance with a short TTL — rebuilt on every Ctrl+P after 60s.
   private static final long CUST_TTL_MS = 60_000; // 1 minute
   private List<FileEntry> custCache = null;
   private long custCacheBuiltAt = 0;
   private volatile boolean custCacheBuilding = false;
   ...
   private void closeDialog() {
      setVisible(false);
      // Clear cust cache on close so it rebuilds fresh next open.
      // Core cache is intentionally kept — it is static and expensive to rebuild.
      custCache = null;
      custCacheBuiltAt = 0;
   }
    ...
    *
    * Cust cache: rebuilt if null or older than CUST_TTL_MS.
    * Core cache: built once per IDE session (static). Never cleared automatically.
    *
    * If either cache is still building, the existing (possibly stale) data
    * is shown immediately and the UI updates when the build finishes.
    */
   private void ensureCache() {
      long now = System.currentTimeMillis();

      boolean custReady = custCache != null && (now - custCacheBuiltAt) < CUST_TTL_MS;
      ...
      // Show stale results immediately while rebuilding
      ...
      new Thread(() -> {
         try {
            ...
            SwingUtilities.invokeLater(() -> {
               custCache = entries;
               custCacheBuiltAt = System.currentTimeMillis();
      ...
      for (File f : children) {
         ...
         if (f.isDirectory()) {
            ...
            if (name.equals(".git") || name.equals(".svn")
                    ...
                    || name.equals("lib") || name.equals("var")
                    || name.equals("config") || name.equals("update")) {
               continue;
            }
