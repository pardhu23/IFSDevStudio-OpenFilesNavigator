...
public final class QuickFileSearchDialog extends JDialog {
    ...
    private static final class BatchListModel<E> extends AbstractListModel<E> {
        ...
        @Override public E      getElementAt(int i) { return data.get(i); }
    ...
    public static void showDialog() { showDialog(null); }
    ...
    private static void scheduleCacheBuilds(Runnable onAllDone) {
        ...
        if (!needCust && !needBuild && !needCore) {
            if (onAllDone != null) SwingUtilities.invokeLater(onAllDone);
            return;
        }
        ...
        if (onAllDone != null) {
            ...
            Runnable onOneDone = () -> {
                if (pending.decrementAndGet() == 0) onAllDone.run();
            };
            if (needCust)  buildCustCacheAsync(onOneDone);
            if (needBuild) buildBuildCacheAsync(onOneDone);
            if (needCore)  buildCoreCacheAsync(onOneDone);
        } else {
            if (needCust)  buildCustCacheAsync(null);
            if (needBuild) buildBuildCacheAsync(null);
            if (needCore)  buildCoreCacheAsync(null);
        }
    }
    ...
    private static List<FileEntry> tryLoadCoreDiskCache(Set<String> roots, Set<String> exts) {
        ...
        try (DataInputStream in = new DataInputStream(
            ...
            for (int i = 0; i < numRoots; i++) storedRoots.put(in.readUTF(), in.readLong());
    ...
    private static void writeCustDirtyFlag() {
        try { custDirtyFlagFile().createNewFile(); } catch (IOException ignored) {}
    }
     ...
     *
     * <p>FIX vs old code: the callback is stored in {@code pendingMergeCallback}
    ...
    private static void scheduleMergedRebuild(Runnable extraCallback) {
        ...
        if (extraCallback != null) pendingMergeCallback = extraCallback;
    ...
    private static void walkParallel(File dir, String rootPath,
        ...
        if (!topTasks.isEmpty()) {
            WALK_POOL.invoke(new RecursiveAction() {
                @Override protected void compute() { invokeAll(topTasks); }
            });
        }
    ...
    private static final class WalkTask extends RecursiveAction {
        ...
        WalkTask(File dir, String rootPath, ConcurrentLinkedQueue<FileEntry> result,
            ...
            this.dir = dir; this.rootPath = rootPath;
            this.result = result; this.source = source; this.exts = exts;
        }

        @Override protected void compute() { walkDir(dir, source); }
    ...
    private void buildUI() {
        ...
        searchField.getDocument().addDocumentListener(new DocumentListener() {
            @Override public void insertUpdate(DocumentEvent e)  { scheduleQueryUpdate(); }
            @Override public void removeUpdate(DocumentEvent e)  { scheduleQueryUpdate(); }
            @Override public void changedUpdate(DocumentEvent e) { scheduleQueryUpdate(); }
        });
     ...
     *
     * <p>FIX vs old code: delegates entirely to {@code scheduleCacheBuilds()} which
